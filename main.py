...
# 4. Define parsing function (example implementation below — you should replace this with actual logic from your original script)
...
def preprocess_statement_text(text: str) -> str:
    """Clean and merge multiline transactions"""
    # Remove multiple newlines
    text = re.sub(r'\n\s*\n', '\n', text)

    # Merge ref no continuation lines (typical in your statement)
    text = re.sub(r'\n(-[A-Z0-9]+.*?)\n', r' \1\n', text)

    # Merge narration spills to next line (common in UPI/NEFT)
...
def extract_text_from_pdf(pdf_file: bytes) -> str:
    ...
    try:
        with fitz.open(stream=pdf_file, filetype="pdf") as doc:
            ...
            # If PyMuPDF extracts no text, try OCR
            if not text.strip():
                print("Trying OCR...")
...
def parse_pdf_statement(pdf_file: bytes) -> List[Dict[str, Any]]:
    ...
    if bank_format == "hdfc":
        # ✅ Using balance-difference method (robust for HDFC)
        pattern = re.compile(
        ...
        # Use already extracted text from PyMuPDF
        ...
        for line in lines:
            try:
                # Match typical ICICI mini statement format
      ...
      with pdfplumber.open("KVB Statement.pdf") as pdf:
                  ...
                  for table in tables:
                      for row in table:
                          if row and len(row) >= 5:
                              ...
                              # Check date format (KVB uses dd/mm/yyyy)
...
# 5. Determine file type and process accordingly
# Database Functions
